...
foreach($checkbox as $checkboxid => $_id){
	...
	$resultzaiku=mysqli_query($conn,"SELECT quantity,_id FROM `t_inout` WHERE (outquantity is null OR outquantity = 0 OR quantity-outquantity>0) AND banngo='$row[1]' AND asahipo='$row[2]' AND campany like '%$row[4]%' order by quantity desc");
